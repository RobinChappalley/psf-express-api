...
export async function seedUsers() {
  try {
    ...
    const accompagnants = []
    for (const u of accompagnantsData) {
      accompagnants.push(await User.create(u))
    }
    ...
    const parents = []
    for (const u of parentsData) {
      parents.push(await User.create(u))
    }
    ...
    const enfants = []
    for (const u of enfantsData) {
      enfants.push(await User.create(u))
    }
